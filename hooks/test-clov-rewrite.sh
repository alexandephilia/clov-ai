#!/bin/bash
# Test suite for clov-rewrite.sh
# Feeds mock JSON through the hook and verifies the rewritten commands.
#
# Usage: bash ~/.claude/hooks/test-clov-rewrite.sh

HOOK="${HOOK:-$HOME/.claude/hooks/clov-rewrite.sh}"
PASS=0
FAIL=0
TOTAL=0
...
echo "============================================"
echo "  CLOV Rewrite Hook Test Suite"
echo "============================================"
echo ""
...
test_rewrite "env + playwright" \
  ...
  "TEST_SESSION_ID=2 clov playwright test --config=foo"

test_rewrite "env + git status" \
  "GIT_PAGER=cat git status" \
  "GIT_PAGER=cat clov git status"

test_rewrite "env + git log" \
  "GIT_PAGER=cat git log --oneline -10" \
  "GIT_PAGER=cat clov git log --oneline -10"

test_rewrite "multi env + vitest" \
  "NODE_ENV=test CI=1 npx vitest run" \
  "NODE_ENV=test CI=1 clov vitest run"
...
test_rewrite "env + npm run" \
  ...
  "NODE_ENV=test clov npm test:e2e"
...
test_rewrite "env + docker compose logs (supported, rewritten)" \
  ...
  "COMPOSE_PROJECT_NAME=test clov docker compose logs web"
...
test_rewrite "docker compose up -d (NOT rewritten — unsupported by clov)" \
  "docker compose up -d" \
  ""

test_rewrite "docker compose logs postgrest" \
  "docker compose logs postgrest" \
  "clov docker compose logs postgrest"

test_rewrite "docker compose ps" \
  "docker compose ps" \
  "clov docker compose ps"

test_rewrite "docker compose build" \
  "docker compose build" \
  "clov docker compose build"

test_rewrite "docker compose down (NOT rewritten — unsupported by clov)" \
  "docker compose down" \
  ""
...
test_rewrite "docker run --rm postgres" \
  ...
  "clov docker run --rm postgres"

test_rewrite "docker exec -it db psql" \
  "docker exec -it db psql" \
  "clov docker exec -it db psql"
...
test_rewrite "vitest run --reporter" \
  ...
  "clov vitest run --reporter=verbose"
...
echo "--- Audit logging (CLOV_HOOK_AUDIT=1) ---"
...
test_audit_log() {
  ...
  local input_json
  input_json=$(jq -n --arg cmd "$input_cmd" '{"tool_name":"Bash","tool_input":{"command":$cmd}}')
  echo "$input_json" | CLOV_HOOK_AUDIT=1 CLOV_AUDIT_DIR="$AUDIT_TMPDIR" bash "$HOOK" 2>/dev/null || true
...
test_audit_log "audit: skip already_clov" \
  "clov git status" \
  "skip:already_clov"
...
# Test no log when CLOV_HOOK_AUDIT is unset
rm -f "$AUDIT_TMPDIR/hook-audit.log"
input_json=$(jq -n --arg cmd "git status" '{"tool_name":"Bash","tool_input":{"command":$cmd}}')
echo "$input_json" | CLOV_AUDIT_DIR="$AUDIT_TMPDIR" bash "$HOOK" 2>/dev/null || true
TOTAL=$((TOTAL + 1))
if [ ! -f "$AUDIT_TMPDIR/hook-audit.log" ]; then
  printf "  ${GREEN}PASS${RESET} audit: no log when CLOV_HOOK_AUDIT unset\n"
  PASS=$((PASS + 1))
else
  printf "  ${RED}FAIL${RESET} audit: log created when CLOV_HOOK_AUDIT unset\n"
  FAIL=$((FAIL + 1))
fi
