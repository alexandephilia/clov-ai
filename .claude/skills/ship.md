...
Systematic release workflow for CLOV: build verification, version bump, changelog update, git tag, and push to trigger CI/CD.
...
```bash
hyperfine 'target/release/clov git status' --warmup 3
# Should show <10ms mean time

/usr/bin/time -l target/release/clov git status
...
- **MAJOR** (v1.0.0): Breaking changes (rare for CLOV)
- **MINOR** (v0.X.0): New features, new filters, new commands
- **PATCH** (v0.0.X): Bug fixes, performance improvements

**Examples**:
- New filter added (`clov pytest`) → **MINOR** bump (v0.16.0 → v0.17.0)
...
### Added
- `clov pytest` command for Python test filtering (90% token reduction)
...
Added:
- clov pytest command (90% token reduction)
...
target/release/clov --version | grep "$NEW_VERSION"
...
**Symptom**: `clov --version` shows old version after bump
