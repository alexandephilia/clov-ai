...
//!
//! This module provides comprehensive tracking of CLOV command executions,
//! recording token savings, execution times, and providing aggregation APIs
//! for daily/weekly/monthly statistics.
//!
//! # Architecture
//!
//! - Storage: SQLite database (~/.local/share/clov/tracking.db)
...
impl Tracker {
    ...
    fn get_by_command(
        ...
        let (project_exact, project_glob) = project_filter_params(project_path); // added
        let mut stmt = self.conn.prepare(
            "SELECT clov_cmd, COUNT(*), SUM(saved_tokens), AVG(savings_pct), AVG(exec_time_ms)
             FROM commands
    ...
    pub fn get_recent_filtered(
        ...
        let (project_exact, project_glob) = project_filter_params(project_path); // added
        let mut stmt = self.conn.prepare(
            "SELECT timestamp, clov_cmd, saved_tokens, savings_pct
             FROM commands
             WHERE (?1 IS NULL OR project_path = ?1 OR project_path GLOB ?2)
             ORDER BY timestamp DESC
...
fn get_db_path() -> Result<PathBuf> {
    // Priority 1: Environment variable CLOV_DB_PATH
    if let Ok(custom_path) = std::env::var("CLOV_DB_PATH") {
    ...
    Ok(data_dir.join("clov").join("history.db"))
}
...
///
/// ```
/// use clov::tracking::estimate_tokens;
///
...
impl TimedExecution {
    ...
    /// let timer = TimedExecution::start();
    /// // ... execute command ...
    /// timer.track("cmd", "clov cmd", "input", "output");
    /// ```
    pub fn start() -> Self {
        Self {
            start: Instant::now(),
    ...
    /// - `clov_cmd`: CLOV command used (e.g., "clov git tag --list")
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// timer.track_passthrough("git tag", "clov git tag");
    /// ```
    pub fn track_passthrough(&self, original_cmd: &str, clov_cmd: &str) {
        ...
        if let Ok(tracker) = Tracker::new() {
            let _ = tracker.record(original_cmd, clov_cmd, 0, 0, elapsed_ms);
        }
    }
}
...
/// ```
/// use std::ffi::OsString;
/// use clov::tracking::args_display;
///
...
mod tests {
    ...
    fn test_tracker_record_and_recent() {
        ...
        let test_cmd = format!("clov git status test_{}", std::process::id());
    ...
    fn test_track_passthrough_no_dilution() {
        ...
        let cmd1 = format!("clov cmd1_test_{}", pid);
        let cmd2 = format!("clov cmd2_passthrough_test_{}", pid);
        ...
        let record1 = recent
            .iter()
            .find(|r| r.clov_cmd == cmd1)
            .expect("cmd1 record not found");
        let record2 = recent
            .iter()
            .find(|r| r.clov_cmd == cmd2)
    ...
    fn test_timed_execution_passthrough() {
        let timer = TimedExecution::start();
        timer.track_passthrough("git tag", "clov git tag (passthrough)");
        ...
        let pt = recent
            .iter()
            .find(|r| r.clov_cmd.contains("passthrough"))
            .expect("Passthrough record not found");
    ...
    // 7. get_db_path respects environment variable CLOV_DB_PATH
    #[test]
    fn test_custom_db_path_env() {
        use std::env;

        let custom_path = "/tmp/clov_test_custom.db";
        env::set_var("CLOV_DB_PATH", custom_path);

        let db_path = get_db_path().expect("Failed to get db path");
        assert_eq!(db_path, PathBuf::from(custom_path));

        env::remove_var("CLOV_DB_PATH");
    }
    ...
    fn test_default_db_path() {
        ...
        let db_path = get_db_path().expect("Failed to get db path");
        assert!(db_path.ends_with("clov/history.db"));
    }
