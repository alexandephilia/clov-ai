...
fn list_prs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    timer.track("gh pr list", "clov gh pr list", &raw, &filtered);
    Ok(())
}
...
fn view_pr(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("clov gh pr view {}", pr_number),
            &stderr,
            &stderr,
        );
...
fn pr_checks(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("clov gh pr checks {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_status(_verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh pr status", "clov gh pr status", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr status", "clov gh pr status", &raw, &filtered);
    Ok(())
}
...
fn list_issues(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh issue list", "clov gh issue list", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh issue list", "clov gh issue list", &raw, &filtered);
    Ok(())
}
...
fn view_issue(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("clov gh issue view {}", issue_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn list_runs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    timer.track("gh run list", "clov gh run list", &raw, &filtered);
    Ok(())
}
...
fn view_run(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("clov gh run view {}", run_id),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn run_repo(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh repo view", "clov gh repo view", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh repo view", "clov gh repo view", &raw, &filtered);
    Ok(())
}
...
fn pr_create(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr create", "clov gh pr create", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr create", "clov gh pr create", &stdout, &filtered);
    Ok(())
}
...
fn pr_merge(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr merge", "clov gh pr merge", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr merge", "clov gh pr merge", &raw, &filtered);
    Ok(())
}
...
fn pr_diff(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh pr diff", "clov gh pr diff", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr diff", "clov gh pr diff", &raw, &filtered);
    Ok(())
}
...
fn run_api(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh api", "clov gh api", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh api", "clov gh api", &raw, &filtered);
    Ok(())
}
...
fn run_passthrough_with_extra(cmd: &str, base_args: &[&str], extra_args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(&full_cmd, &format!("clov {} (passthrough)", full_cmd));
...
fn run_passthrough(cmd: &str, subcommand: &str, args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("clov {} {} {} (passthrough)", cmd, subcommand, args_str),
    );
