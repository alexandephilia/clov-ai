...
pub fn run_stdout(url: &str, args: &[String], verbose: u8) -> Result<()> {
    ...
    if output.status.success() {
        ...
        let mut clov_output = String::new();
        if total > 20 {
            clov_output.push_str(&format!(
                "⬇️ {} ok | {} lines | {}\n",
                compact_url(url),
                total,
                format_size(output.stdout.len() as u64)
            ));
            clov_output.push_str("--- first 10 lines ---\n");
            for line in lines.iter().take(10) {
                clov_output.push_str(&format!("{}\n", truncate_line(line, 100)));
            }
            clov_output.push_str(&format!("... +{} more lines", total - 10));
        } else {
            clov_output.push_str(&format!("⬇️ {} ok | {} lines\n", compact_url(url), total));
            for line in &lines {
                clov_output.push_str(&format!("{}\n", line));
            }
        }
