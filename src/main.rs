...
#[command(
    name = "clov",
    version,
    about = "Clov Token Omitter - Minimize LLM token consumption",
...
enum Commands {
    ...
    Init {
        ...
        /// Remove all CLOV artifacts (hook, CLOV.md, CLAUDE.md reference, settings.json entry)
        #[arg(long)]
        uninstall: bool,
    },
    ...
    /// Rewrite a raw command to its CLOV equivalent (single source of truth for hooks)
    ///
...
fn run_fallback(parse_error: clap::Error) -> Result<()> {
    ...
    // No args → show Clap's error (user ran just "clov" with bad syntax)
    if args.is_empty() {
        parse_error.exit();
    }
    ...
    eprintln!("[clov: parse failed, running raw]");
    ...
    match status {
        Ok(s) => {
            timer.track_passthrough(&raw_command, &format!("clov fallback: {}", raw_command));
        ...
        Err(e) => {
            ...
            eprintln!("[clov: fallback failed: {}]", e);
            parse_error.exit();
        }
    }
    ...
    match cli.command {
            ...
            match args[0].as_str() {
                ...
                "prisma" => {
                    ...
                    if args.len() > 1 {
                        ...
                        match args[1].as_str() {
                            ...
                            _ => {
                                ...
                                timer.track_passthrough(
                                    &format!("npx {}", args_str),
                                    &format!("clov npx {} (passthrough)", args_str),
                                );
                                if !status.success() {
                        ...
                        timer.track_passthrough("npx prisma", "clov npx prisma (passthrough)");
                        if !status.success() {
                            std::process::exit(status.code().unwrap_or(1));
                        }
                    }
        ...
        Commands::Proxy { args } => {
            ...
            timer.track(
                ...
                &format!("clov passthrough {} {}", cmd_name, cmd_args.join(" ")),
                &full_output,
                &full_output,
            );
...
mod tests {
    ...
    fn test_git_commit_single_message() {
        let cli = Cli::try_parse_from(["clov", "git", "commit", "-m", "fix: typo"]).unwrap();
        match cli.command {
            Commands::Git {
    ...
    fn test_git_global_options_parsing() {
        let cli =
            Cli::try_parse_from(["clov", "git", "--no-pager", "--no-optional-locks", "status"])
                .unwrap();
        match cli.command {
            Commands::Git {
                no_pager,
    ...
    fn test_try_parse_version_is_display_version() {
        match Cli::try_parse_from(["clov", "--version"]) {
    ...
    fn test_try_parse_unknown_subcommand_is_error() {
        match Cli::try_parse_from(["clov", "nonexistent-command"]) {
    ...
    fn test_try_parse_git_with_dash_c_succeeds() {
        ...
        let result = Cli::try_parse_from(["clov", "git", "-C", "/path", "status"]);
    ...
    #[test]
    fn test_new_primary_command_names_parse() {
        let cli = Cli::try_parse_from(["clov", "pulse", "--graph"]).unwrap();
        match cli.command {
            Commands::Gain { graph, .. } => assert!(graph),
            _ => panic!("Expected Gain command"),
        }

        let cli = Cli::try_parse_from(["clov", "hook", "--show"]).unwrap();
        match cli.command {
            Commands::Init { show, .. } => assert!(show),
            _ => panic!("Expected Init command"),
        }

        let cli =
            Cli::try_parse_from(["clov", "bridge", "proxy", "npx", "-y", "mcp-remote"]).unwrap();
        match cli.command {
            Commands::Mcp {
                action: McpAction::Proxy { command, args, .. },
            } => {
    ...
    fn test_meta_commands_reject_bad_flags() {
        // CLOV meta-commands should produce parse errors (not fall through to raw execution).
        // Skip "proxy" because it uses trailing_var_arg (accepts any args by design).
        for cmd in CLOV_META_COMMANDS {
            if *cmd == "proxy" {
                continue;
            }
            let result = Cli::try_parse_from(["clov", cmd, "--nonexistent-flag-xyz"]);
            assert!(
                result.is_err(),
