...
pub fn run(
    ...
    if summary.total_commands == 0 {
        ...
        println!("Run some clov commands to start tracking savings.");
        return Ok(());
    }
        ...
        if history {
            ...
            if !recent.is_empty() {
                ...
                for rec in recent {
                    ...
                    let cmd_short = if rec.clov_cmd.len() > 25 {
                        format!("{}...", &rec.clov_cmd[..22])
                    } else {
                        rec.clov_cmd.clone()
                    };
...
fn show_failures(tracker: &Tracker) -> Result<()> {
    ...
    println!("{}", styled("CLOV Parse Failures", true));
    println!("{}", "═".repeat(60));
    println!();
