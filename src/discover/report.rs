...
/// CLOV support status for a command.
...
pub enum ClovStatus {
    ...
    Passthrough,
    /// CLOV doesn't handle this command at all
    NotSupported,
}
...
impl ClovStatus {
    pub fn as_str(&self) -> &'static str {
        match self {
            ClovStatus::Existing => "existing",
            ClovStatus::Passthrough => "passthrough",
            ClovStatus::NotSupported => "not-supported",
        }
    }
}

/// A supported command that CLOV already handles.
#[derive(Debug, Serialize)]
pub struct SupportedEntry {
    pub command: String,
    pub count: usize,
    pub clov_equivalent: &'static str,
    pub category: &'static str,
    pub estimated_savings_tokens: usize,
    pub estimated_savings_pct: f64,
    pub clov_status: ClovStatus,
}

/// An unsupported command not yet handled by CLOV.
#[derive(Debug, Serialize)]
...
pub fn format_text(report: &DiscoverReport, limit: usize, verbose: bool) -> String {
    ...
    out.push_str("CLOV Discover -- Savings Opportunities\n");
    ...
    out.push_str(&format!(
        "Already using CLOV: {} commands ({}%)\n",
        report.already_clov,
        if report.total_commands > 0 {
            report.already_clov * 100 / report.total_commands
        } else {
            0
        }
    ));
    ...
    if report.supported.is_empty() && report.unsupported.is_empty() {
        out.push_str("\nNo missed savings found. CLOV usage looks good!\n");
        return out;
    }
    ...
    if !report.supported.is_empty() {
        ...
        out.push_str(&format!(
            "{:<24} {:>5}    {:<18} {:<13} {:>12}\n",
            "Command", "Count", "CLOV Equivalent", "Status", "Est. Savings"
        ));
        ...
        for entry in report.supported.iter().take(limit) {
            out.push_str(&format!(
                ...
                entry.count,
                entry.clov_equivalent,
                entry.clov_status.as_str(),
