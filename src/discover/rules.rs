use super::report::ClovStatus;

/// A rule mapping a shell command pattern to its CLOV equivalent.
pub struct ClovRule {
    pub clov_cmd: &'static str,
    /// Original command prefixes to replace with clov_cmd (longest first for correct matching).
    pub rewrite_prefixes: &'static [&'static str],
    pub category: &'static str,
    pub savings_pct: f64,
    pub subcmd_savings: &'static [(&'static str, f64)],
    pub subcmd_status: &'static [(&'static str, ClovStatus)],
}
...
pub const RULES: &[ClovRule] = &[
    ...
    ClovRule {
        ...
        subcmd_status: &[("fmt", ClovStatus::Passthrough)],
    },
    ClovRule {
        clov_cmd: "clov pnpm",
    ...
    ClovRule {
        // "next build" is stripped to "clov next" — the build subcommand is internal
        clov_cmd: "clov next",
...
/// Commands to ignore (shell builtins, trivial, already clov).
