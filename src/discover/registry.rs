...
pub enum Classification {
    Supported {
        clov_equivalent: &'static str,
        category: &'static str,
        estimated_savings_pct: f64,
        status: super::report::ClovStatus,
    },
    Unsupported {
        base_command: String,
    },
        ...
        let (savings, status) = if let Some(caps) = COMPILED[idx].captures(cmd_clean) {
            if let Some(sub) = caps.get(1) {
                ...
                let status = rule
                    ...
                    .find(|(s, _)| *s == subcmd)
                    .map(|(_, st)| *st)
                    .unwrap_or(super::report::ClovStatus::Existing);
                ...
                (rule.savings_pct, super::report::ClovStatus::Existing)
            }
        } else {
            (rule.savings_pct, super::report::ClovStatus::Existing)
        };
...
/// Rewrite a raw command to its CLOV equivalent.
///
/// Returns `Some(rewritten)` if the command has an CLOV equivalent or is already CLOV.
...
pub fn rewrite_command(cmd: &str) -> Option<String> {
    ...
    // Simple (non-compound) already-CLOV command — return as-is.
    // For compound commands that start with "clov" (e.g. "clov git add . && cargo test"),
    ...
    let has_compound = trimmed.contains("&&")
        ...
        || trimmed.contains('|')
        || trimmed.contains(" & ");
    if !has_compound && (trimmed.starts_with("clov ") || trimmed == "clov") {
...
/// Returns `Some(rewritten)` if matched (including already-CLOV pass-through).
...
fn rewrite_segment(seg: &str) -> Option<String> {
    ...
    // Already CLOV — pass through unchanged
    if trimmed.starts_with("clov ") || trimmed == "clov" {
        return Some(trimmed.to_string());
    }
    ...
    // Find the matching rule (clov_cmd values are unique across all rules)
    let rule = RULES.iter().find(|r| r.clov_cmd == clov_equivalent)?;
    ...
    for &prefix in rule.rewrite_prefixes {
        if let Some(rest) = strip_word_prefix(cmd_clean, prefix) {
            let rewritten = if rest.is_empty() {
                format!("{}{}", env_prefix, rule.clov_cmd)
            } else {
                format!("{}{} {}", env_prefix, rule.clov_cmd, rest)
            };
...
#[cfg(test)]
mod tests {
    use super::super::report::ClovStatus;
    use super::*;
    ...
    #[test]
    fn test_classify_clov_already() {
        assert_eq!(classify_command("clov git status"), Classification::Ignored);
    }
    ...
    fn test_rewrite_compound_and() {
        assert_eq!(
            ...
            Some("clov git add . && clov cargo test".into())
        );
    }
    ...
    #[test]
    fn test_rewrite_already_clov() {
        assert_eq!(
            rewrite_command("clov git status"),
            Some("clov git status".into())
        );
    }
    ...
    fn test_rewrite_background_single_amp() {
        assert_eq!(
            ...
            Some("clov cargo test & clov git status".into())
        );
    }
    ...
    fn test_rewrite_background_unsupported_right() {
        assert_eq!(
            ...
            Some("clov cargo test & terraform plan".into())
        );
    }
    ...
    fn test_rewrite_with_env_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh clov git push".into())
        );
    }
    ...
    fn test_rewrite_npx_playwright() {
        assert_eq!(
            ...
            Some("clov playwright test".into())
        );
    }
    ...
    fn test_rewrite_pipe_first_only() {
        ...
        assert_eq!(
            ...
            Some("clov git log -10 | grep feat".into())
        );
    }
    ...
    fn test_rewrite_mixed_compound_partial() {
        // First segment already CLOV, second gets rewritten
        assert_eq!(
            rewrite_command("clov git add . && cargo test"),
            Some("clov git add . && clov cargo test".into())
        );
    }
    ...
    fn test_classify_cargo_install() {
        assert!(matches!(
            classify_command("cargo install clov"),
            Classification::Supported {
                clov_equivalent: "clov cargo",
                ..
            }
    ...
    fn test_rewrite_gh_release() {
        assert_eq!(
            ...
            Some("clov gh release list".into())
        );
    }
    ...
    fn test_rewrite_cargo_install() {
        assert_eq!(
            rewrite_command("cargo install clov"),
            Some("clov cargo install clov".into())
        );
    }
    ...
    fn test_rewrite_kubectl_describe() {
        assert_eq!(
            ...
            Some("clov kubectl describe pod mypod".into())
        );
    }
    ...
    fn test_rewrite_docker_run() {
        assert_eq!(
            ...
            Some("clov docker run --rm ubuntu bash".into())
        );
    }
    ...
    #[test]
    fn test_rewrite_aws() {
        assert_eq!(rewrite_command("aws s3 ls"), Some("clov aws s3 ls".into()));
    }
    ...
    fn test_rewrite_aws_ec2() {
        assert_eq!(
            ...
            Some("clov aws ec2 describe-instances --region us-east-1".into())
        );
    }
    ...
    fn test_rewrite_psql() {
        assert_eq!(
            ...
            Some("clov psql -U postgres -d mydb".into())
        );
    }
    ...
    fn test_rewrite_ruff_format() {
        assert_eq!(
            ...
            Some("clov ruff format src/".into())
        );
    }
    ...
    fn test_rewrite_python_m_pytest() {
        assert_eq!(
            ...
            Some("clov pytest -x tests/".into())
        );
    }

    #[test]
    fn test_rewrite_pip_list() {
        assert_eq!(rewrite_command("pip list"), Some("clov pip list".into()));
    }
    ...
    fn test_rewrite_pip_outdated() {
        assert_eq!(
            ...
            Some("clov pip outdated".into())
        );
    }

    #[test]
    fn test_rewrite_uv_pip_list() {
        assert_eq!(rewrite_command("uv pip list"), Some("clov pip list".into()));
    }
    ...
    fn test_classify_golangci_lint() {
        assert!(matches!(
            ...
            Classification::Supported {
                clov_equivalent: "clov golangci-lint",
                ..
            }
        ));
    }
    ...
    fn test_rewrite_golangci_lint() {
        assert_eq!(
            ...
            Some("clov golangci-lint run ./...".into())
        );
    }
    ...
    fn test_rewrite_prisma() {
        assert_eq!(
            ...
            Some("clov prisma migrate dev".into())
        );
    }
    ...
    fn test_rewrite_prettier() {
        assert_eq!(
            ...
            Some("clov prettier --check src/".into())
        );
    }

    #[test]
    fn test_rewrite_pnpm_list() {
        assert_eq!(rewrite_command("pnpm list"), Some("clov pnpm list".into()));
    }
    ...
    fn test_rewrite_compound_or() {
        ...
        assert_eq!(
            ...
            Some("clov cargo test || clov cargo build".into())
        );
    }
    ...
    fn test_rewrite_compound_semicolon() {
        assert_eq!(
            ...
            Some("clov git status; clov cargo test".into())
        );
    }
    ...
    fn test_rewrite_compound_pipe_raw_filter() {
        ...
        assert_eq!(
            ...
            Some("clov cargo test | grep FAILED".into())
        );
    }
    ...
    fn test_rewrite_compound_four_segments() {
        assert_eq!(
            ...
            Some(
                "clov cargo fmt --all && clov cargo clippy && clov cargo test && clov git status"
                    .into()
            )
        );
    }
    ...
    fn test_rewrite_env_var_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh clov git push origin main".into())
        );
    }
    ...
    // --- All RULES have non-empty clov_cmd and at least one rewrite_prefix ---

    #[test]
    fn test_all_rules_have_valid_clov_cmd() {
        for rule in RULES {
            assert!(!rule.clov_cmd.is_empty(), "Rule with empty clov_cmd found");
            assert!(
                rule.clov_cmd.starts_with("clov "),
                "clov_cmd '{}' must start with 'clov '",
                rule.clov_cmd
            );
            assert!(
