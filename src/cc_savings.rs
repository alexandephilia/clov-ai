...
pub struct PeriodSavings {
    ...
    // clov metrics
    pub clov_commands: Option<usize>,
    pub clov_saved_tokens: Option<usize>,
    pub clov_savings_pct: Option<f64>,
...
impl PeriodSavings {
    fn new(label: &str) -> Self {
        Self {
            ...
            clov_commands: None,
            clov_saved_tokens: None,
            clov_savings_pct: None,
    ...
    fn set_clov_from_day(&mut self, stats: &DayStats) {
        self.clov_commands = Some(stats.commands);
        self.clov_saved_tokens = Some(stats.saved_tokens);
        self.clov_savings_pct = Some(stats.savings_pct);
    }

    fn set_clov_from_week(&mut self, stats: &WeekStats) {
        self.clov_commands = Some(stats.commands);
        self.clov_saved_tokens = Some(stats.saved_tokens);
        self.clov_savings_pct = Some(stats.savings_pct);
    }

    fn set_clov_from_month(&mut self, stats: &MonthStats) {
        self.clov_commands = Some(stats.commands);
        self.clov_saved_tokens = Some(stats.saved_tokens);
        self.clov_savings_pct = Some(if stats.input_tokens + stats.output_tokens > 0 {
    ...
    fn compute_weighted_metrics(&mut self) {
        ...
        if let (Some(cost), Some(saved)) = (self.cc_cost, self.clov_saved_tokens) {
...
struct Totals {
    ...
    clov_commands: usize,
    clov_saved_tokens: usize,
    clov_avg_savings_pct: f64,
...
fn merge_weekly(cc: Option<Vec<CcusagePeriod>>, clov: Vec<WeekStats>) -> Vec<PeriodSavings> {
    ...
    // Merge clov data (week_start = legacy Saturday "2026-01-18")
...
/// Convert Saturday week_start (legacy clov) to ISO Monday
...
fn convert_saturday_to_monday(saturday: &str) -> Option<String> {
    ...
    // clov uses Saturday as week start, ISO uses Monday
...
fn compute_totals(periods: &[PeriodSavings]) -> Totals {
    ...
    for p in periods {
        ...
        if let Some(cmds) = p.clov_commands {
            totals.clov_commands += cmds;
        }
        if let Some(saved) = p.clov_saved_tokens {
            totals.clov_saved_tokens += saved;
        }
        if let Some(pct) = p.clov_savings_pct {
            pct_sum += pct;
            pct_count += 1;
        }
    }
    ...
    if pct_count > 0 {
        totals.clov_avg_savings_pct = pct_sum / pct_count as f64;
    }
    ...
    if weighted_units > 0.0 {
        ...
        totals.savings_weighted = Some(totals.clov_saved_tokens as f64 * input_cpt);
    }
    ...
    if totals.cc_active_tokens > 0 {
        ...
        totals.savings_active = Some(totals.clov_saved_tokens as f64 * totals.active_cpt.unwrap());
    }
...
fn display_summary(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    println!("  CLOV commands:                 {}", totals.clov_commands);
    println!(
        "  Tokens saved:                 {}",
        format_tokens(totals.clov_saved_tokens)
    );
    println!();
    ...
    println!("  CLOV compresses CLI outputs before they enter Claude's context.");
...
fn print_period_table(periods: &[PeriodSavings], verbose: u8) {
    ...
    if verbose > 0 {
        ...
        println!(
            ...
            "Period", "Spent", "Saved", "Savings", "Active$", "Blended$", "CLOV Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<10} {:-<12} {:-<12}",
            "", "", "", "", "", "", ""
        ...
        println!(
            "{:<12} {:>10} {:>10} {:>10} {:>12}",
            "Period", "Spent", "Saved", "Savings", "CLOV Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<12}",
            "", "", "", "", ""
...
fn export_json(
    ...
    if all || daily {
        ...
        export.daily = Some(merge_daily(cc, clov));
    }
    ...
    if all || weekly {
        ...
        export.weekly = Some(merge_weekly(cc, clov));
    }
    ...
    if all || monthly {
        ...
        let periods = merge_monthly(cc, clov);
...
fn export_csv(
    ...
    println!("period,spent,input_tokens,output_tokens,cache_create,cache_read,active_tokens,total_tokens,saved_tokens,weighted_savings,active_savings,blended_savings,clov_commands");
    ...
    if all || daily {
        ...
        let clov = tracker
            .get_all_days()
            .context("Failed to load daily token savings for JSON export")?;
        let periods = merge_daily(cc, clov);
        for p in periods {
            print_csv_row(&p);
        }
    }
    ...
    if all || weekly {
        ...
        let clov = tracker
            .get_by_week()
            .context("Failed to load weekly token savings for export")?;
        let periods = merge_weekly(cc, clov);
        for p in periods {
            print_csv_row(&p);
        }
    }
...
fn print_csv_row(p: &PeriodSavings) {
    ...
    let blended_savings = p
        ...
        .map(|s| format!("{:.4}", s))
        .unwrap_or_default();
    let cmds = p.clov_commands.map(|c| c.to_string()).unwrap_or_default();
...
mod tests {
    ...
    fn test_merge_monthly_both_present() {
        ...
        let merged = merge_monthly(Some(cc), clov);
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].label, "2026-01");
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert_eq!(merged[0].clov_commands, Some(10));
    }
    ...
    fn test_merge_monthly_only_ccusage() {
        ...
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert!(merged[0].clov_commands.is_none());
    }

    #[test]
    fn test_merge_monthly_only_clov() {
        let clov = vec![MonthStats {
            month: "2026-01".to_string(),
            commands: 10,
            input_tokens: 800,
        ...
        let merged = merge_monthly(None, clov);
        assert_eq!(merged.len(), 1);
    ...
    fn test_compute_weighted_input_cpt() {
        ...
        p.clov_saved_tokens = Some(10_000);
    ...
    fn test_compute_totals() {
        let periods = vec![
            PeriodSavings {
                ...
                clov_commands: Some(5),
                clov_saved_tokens: Some(2000),
                clov_savings_pct: Some(50.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
            ...
            PeriodSavings {
                ...
                clov_commands: Some(10),
                clov_saved_tokens: Some(3000),
                clov_savings_pct: Some(60.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
        ...
        assert_eq!(totals.clov_commands, 15);
        assert_eq!(totals.clov_saved_tokens, 5000);
        assert_eq!(totals.clov_avg_savings_pct, 55.0);
