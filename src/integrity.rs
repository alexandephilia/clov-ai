...
//!
//! CLOV installs a PreToolUse hook (`clov-rewrite.sh`) that auto-approves
...
const HASH_FILENAME: &str = ".clov-hook.sha256";
...
pub enum IntegrityStatus {
    ...
    NoBaseline,
    /// Neither hook nor hash file exist (CLOV not installed)
    NotInstalled,
...
/// Resolve the default hook path (~/.claude/hooks/clov-rewrite.sh)
pub fn resolve_hook_path() -> Result<PathBuf> {
    dirs::home_dir()
        .map(|h| h.join(".claude").join("hooks").join("clov-rewrite.sh"))
...
pub fn runtime_check() -> Result<()> {
    match verify_hook()? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            eprintln!("clov: hook integrity check FAILED");
            eprintln!(
                "  Expected hash: {}...",
...
mod tests {
    ...
    fn test_store_and_verify_ok() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("clov-rewrite.sh");
    ...
    fn test_verify_orphaned_hash() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("clov-rewrite.sh");
        let hash_file = temp.path().join(".clov-hook.sha256");

        // Create hash but no hook
        fs::write(
            &hash_file,
            "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2  clov-rewrite.sh\n",
        )
        .unwrap();
    ...
    fn test_store_hash_creates_sha256sum_format() {
        ...
        // Format: "<64 hex chars>  clov-rewrite.sh\n"
        assert!(content.ends_with("  clov-rewrite.sh\n"));
        let parts: Vec<&str> = content.trim().splitn(2, "  ").collect();
        assert_eq!(parts.len(), 2);
        assert_eq!(parts[0].len(), 64);
        assert_eq!(parts[1], "clov-rewrite.sh");
    }
    ...
    fn test_invalid_hash_file_rejected() {
        ...
        fs::write(&hook, "test").unwrap();
        fs::write(&hash_file, "not-a-valid-hash  clov-rewrite.sh\n").unwrap();
