# CLOV Tracking API Documentation

Comprehensive documentation for CLOV's token savings tracking system.
...
CLOV's tracking system records every command execution to provide analytics on token savings. The system:
- Stores command history in SQLite (~/.local/share/clov/tracking.db)
...
```
clov command execution
  ↓
TimedExecution::start()
  ↓
[command runs]
  ↓
TimedExecution::track(original_cmd, clov_cmd, input, output)
  ↓
Tracker::record(original_cmd, clov_cmd, input_tokens, output_tokens, exec_time_ms)
  ↓
SQLite database (~/.local/share/clov/tracking.db)
  ↓
...
- **Linux**: `~/.local/share/clov/tracking.db`
- **macOS**: `~/Library/Application Support/clov/tracking.db`
- **Windows**: `%APPDATA%\clov\tracking.db`
...
pub struct CommandRecord {
    ...
    pub clov_cmd: String,           // CLOV command used
...
impl TimedExecution {
    ...
    pub fn track_passthrough(&self, original_cmd: &str, clov_cmd: &str);
}
```
...
```rust
use clov::tracking::{TimedExecution, Tracker};
...
fn main() -> anyhow::Result<()> {
    ...
    for cmd in recent {
        println!("{}: {} saved {:.1}%",
            cmd.timestamp, cmd.clov_cmd, cmd.savings_pct);
    }
...
```rust
use clov::tracking::TimedExecution;
...
fn main() -> anyhow::Result<()> {
    ...
    timer.track_passthrough("git tag --list", "clov git tag --list");
...
```yaml
# .github/workflows/track-clov-savings.yml
name: Track CLOV Savings
  ...
  track-savings:
    ...
    steps:
      ...
      - name: Post to Slack
        ...
        run: |
          SAVINGS=$(jq -r '.[0].saved_tokens' clov-weekly.json)
          PCT=$(jq -r '.[0].savings_pct' clov-weekly.json)
          curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"📊 CLOV Weekly: ${SAVINGS} tokens saved (${PCT}%)\"}" \
            $SLACK_WEBHOOK
```
...
#!/usr/bin/env python3
"""
Export CLOV metrics to Grafana/Datadog/etc.
"""
import json
import subprocess
from datetime import datetime

def get_clov_metrics():
    """Fetch CLOV metrics as JSON."""
    result = subprocess.run(
        ["clov", "gain", "--all", "--format", "json"],
        capture_output=True,
        text=True
    )
...
- **No telemetry**: CLOV does not phone home or send analytics
- **User control**: Users can delete `~/.local/share/clov/tracking.db` anytime
...
Older databases may not have the `exec_time_ms` column. CLOV automatically migrates on first use, but you can force it:

```bash
sqlite3 ~/.local/share/clov/tracking.db \
  "ALTER TABLE commands ADD COLUMN exec_time_ms INTEGER DEFAULT 0"
```
...
- [COMMAND_AUDIT.md](../claudedocs/COMMAND_AUDIT.md) - List of all CLOV commands
