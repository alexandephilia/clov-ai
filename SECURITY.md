...
CLOV is a CLI tool that executes shell commands and handles user input. PRs from external contributors undergo enhanced security review to protect against:
...
- **`hooks/clov-rewrite.sh`** / **`.claude/hooks/clov-rewrite.sh`** - Thin delegator hook (executes in Claude Code context, intercepts all commands)
