# clov Architecture Documentation

> **clov (Clov Token Omitter)** - A high-performance CLI proxy that minimizes LLM token consumption through intelligent output filtering and compression.

This document provides a comprehensive architectural overview of clov, including system design, data flows, module organization, and implementation patterns.
...
```
┌────────────────────────────────────────────────────────────────────────┐
│                      clov - Token Optimization Proxy                    │
└────────────────────────────────────────────────────────────────────────┘

User Input          CLI Layer           Router            Module Layer
──────────          ─────────           ──────            ────────────

$ clov git log    ─→  Clap Parser   ─→   Commands    ─→   git::run()
...
tracking::track(
    ...
    clov_cmd: "clov git log --oneline -5",
...
Database: ~/.local/share/clov/history.db
```
...
Command                 Raw Time    clov Time    Overhead    Savings
─────────────────────────────────────────────────────────────────────────
...
├─────────┼──────────────────────────────────────────────────────┤
│ 0       │ Success                                              │
│ 1       │ clov internal error (parsing, filtering, etc.)        │
...
1. User Settings (config.toml)
   ───────────────────────────
   Location: ~/.config/clov/config.toml
...
Check existing CLAUDE.md:
  ...
  • else      → ./CLAUDE.md
      ...
      └─ Not exists? → Continue
      ↓
Prompt: "Initialize clov for LLM usage? [y/N]"
      ↓ Yes
Write template:
┌─────────────────────────────────────┐
│ # CLAUDE.md                         │
...
└─────────────────────────────────────┘
      ↓
Success: "✓ Initialized clov for LLM integration"
```
...
**Step-by-step process to add a new clov command:**
...
pub fn run(args: &[String], verbose: u8) -> Result<()> {
    ...
    // Track savings
    tracking::track("mycmd", "clov mycmd", &raw, &filtered);
...
# Build and test
cargo build
./target/debug/clov mycmd arg1 arg2
